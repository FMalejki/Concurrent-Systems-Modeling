...
public class Task1 {
    ...
    static final long RUN_MS = 15_000;
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        long starvationThreshold = RUN_MS / 10;
...
class Buffer {
    ...
    public void produce(String newItem) throws InterruptedException {
        lock.lock();
        try {
            while (count == buffer.length && Task1.running) {
                System.out.println(Thread.currentThread().getName() + " waiting to produce...");
                notFull.await();
            }
            if (!Task1.running && count == buffer.length) return;
            buffer[in] = newItem;
